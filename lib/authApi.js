...
export function newCustomDeck(investigator, name, slots, problem) {
  return newDeck(investigator, name)
    .then(deck => saveDeck(deck.id, deck.name, slots, problem, 0, 0));
}
...
export function saveDeck(id, name, slots, problem, spentXp, xpAdjustment) {
  return getAccessToken().then(accessToken => {
    if (!accessToken) {
      throw new Error('badAccessToken');
    }
