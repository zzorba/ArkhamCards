...
export function useDeckXpStrings(
  ...
  return useMemo(() => {
    ...
    if (parsedDeck.deck?.previousDeckId) {
      const adjustedXp = totalXp
        ? parsedDeck.experience
        : parsedDeck.availableExperience;
      const unspent =
        parsedDeck.availableExperience - (parsedDeck.changes?.spentXp || 0);
      if (unspent === 0) {
        return [xpString(adjustedXp), t`0 unspent`];
      }
      if (unspent < 0) {
        const overspent = Math.abs(unspent);
        return [xpString(adjustedXp), t`${overspent} overspent`];
      }
      const unspentXpStr = unspent;
      return [
        xpString(adjustedXp),
        ngettext(
          msgid`${unspentXpStr} unspent`,
          `${unspentXpStr} unspent`,
          unspent
        ),
      ];
    }
    const adjustedXp = parsedDeck.experience;
    return [xpString(adjustedXp), undefined];
  }, [parsedDeck, totalXp]);
}

export function useSimpleDeckEdits(
  id: DeckId | undefined
): EditDeckState | undefined {
...
export function useDeckEdits(
  ...
  useEffect(() => {
    if (initialDeck && id !== undefined) {
      const editable =
        !initialDeck.owner?.id || !userId || initialDeck.owner.id === userId;
...
function useParsedDeckHelper(
  ...
  const [deckEdits, deckEditsRef] = useDeckEdits(
    id,
    fetchIfMissing ? deck : undefined,
    initialMode
  );
  const tabooSetId =
    deckEdits?.tabooSetChange !== undefined
  ...
  useDebouncedEffect(
    ...
    [cards, deck, listSeperator, deckEdits, visible],
    200
  );
  return {
    deck: deck?.deck,
...
export function useDeckEditState({
  ...
  useEffect(() => {
    ...
    const slotDeltas: {
      ...
      ignoreDeckLimitChanged: false,
      sideChanged: !!find(
        uniq([...keys(deck.sideSlots || {}), ...keys(deckEdits.side)]),
        (code) => {
    ...
    setHasPendingEdits(
      (deckEdits.nameChange && deck.name !== deckEdits.nameChange) ||
        (deckEdits.tagsChange !== undefined &&
          deck.tags !== deckEdits.tagsChange) ||
        (deckEdits.descriptionChange &&
          deck.description_md !== deckEdits.descriptionChange) ||
        (deckEdits.tabooSetChange !== undefined &&
          originalTabooSet !== deckEdits.tabooSetChange) ||
        (deck.previousDeckId &&
          (deck.xp_adjustment || 0) !== deckEdits.xpAdjustment) ||
...
export function useShowDrawWeakness({
  ...
  const saveWeakness = useCallback(
    (code: string, replaceRandomBasicWeakness: boolean) => {
...
function isDirty(originalDeckTags: string[], tags: Toggles) {
  return (
    !!find(originalDeckTags, (t) => !tags[t]) ||
    !!find(
      tags,
      (value, t) => !!value && !find(originalDeckTags, (t2) => t === t2)
    )
  );
}
