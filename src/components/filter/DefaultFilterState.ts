...
function update(
  ...
  return [Math.min(value, minMax[0]), Math.max(value, minMax[1])];
}
...
export async function calculateDefaultDbFilterState(
  ...
  const filterData: CardFilterData = {
    ...
    allFactions: filter(CARD_FACTION_CODES, (code) => factions.has(code)),
...
export function calculateDefaultFilterState(cards: Card[]) {
  const fields = ['xp', 'cost'];
  const locationFields = ['shroud', 'clues'];
  const enemyFields = [
    'health',
    'enemy_damage',
    'enemy_horror',
  ...
  forEach(cards, (card) => {
    ...
    if (card.linked_card) {
      ...
      if (card.linked_card.type_code === 'enemy') {
        ...
        result.enemy_damage = update(
          card.linked_card.enemy_damage,
          result.enemy_damage
        );
        result.enemy_horror = update(
          card.linked_card.enemy_horror,
          result.enemy_horror
        );
        result.enemy_fight = update(
          card.linked_card.enemy_fight,
          result.enemy_fight
        );
        result.enemy_evade = update(
          card.linked_card.enemy_evade,
          result.enemy_evade
        );
      }
    }
