...
export function localizedDate(date: Date, locale: string, noDayOfWeek: boolean = false) {
  if (noDayOfWeek) {
    return format(date, (locale === 'fr' || locale === 'it' || locale === 'pl') ? 'd MMMM yyyy' : 'MMMM d, yyyy', LOCALE_MAP[locale]);
  }
  return format(date, !noDayOfWeek && (locale === 'fr' || locale === 'it' || locale === 'pl') ? 'iiii d MMMM yyyy' : 'MMMM d, yyyy', LOCALE_MAP[locale]);
}
